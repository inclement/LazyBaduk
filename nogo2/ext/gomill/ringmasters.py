...
def interpret_python(source, provided_globals, display_filename):
    ...
    code = compile(source, display_filename, 'exec', division.compiler_flag,
                   True)
    exec(code, result)
    return result
...
class Ringmaster(object):
    ...
    def __init__(self, control_pathname):
        ...
        if ext in (".log", ".status", ".cmd", ".hist", ".report", ".games",
                   ".void", ".gtplogs"):
    ...
    def _load_control_file(self):
        ...
        try:
            ...
            raise ControlFileError("unknown competition type: %s" %
                                   self.competition_type)
    ...
    def set_display_mode(self, presenter_code):
        ...
        if presenter_code not in self._presenter_classes:
            raise RingmasterError("unknown presenter type: %s" %
                                  presenter_code)
    ...
    def load_status(self):
        ...
        try:
            ...
            if (status_format_version != self.status_format_version or
                    status['comp_vn'] !=
                    self.competition.status_format_version):
                raise Exception
    ...
    def _update_display(self):
        ...
        if self.stopping:
            if self.worker_count is None or not self.games_in_progress:
                ...
                p("waiting for workers to finish: %s" % self.stopping_reason)
        ...
        if self.void_game_count > 0:
            print(
                "%d void games; see log file." % self.void_game_count, file=sr)
    ...
    def _prepare_job(self, job):
        ...
        if self.write_gtp_logs:
            job.gtp_log_pathname = os.path.join(self.gtplog_dir_pathname,
                                                "%s.log" % job.game_id)
    ...
    def _get_job(self):
        ...
        if self.games_to_replay:
            ...
            if job.game_id in self.games_in_progress:
                raise RingmasterInternalError("duplicate game id: %s" %
                                              job.game_id)
    ...
    def run(self, max_games=None):
        ...
        try:
            job_manager.run_jobs(
                ...
                passed_exceptions=[
                    RingmasterError, CompetitionError, RingmasterInternalError
                ])
        except KeyboardInterrupt:
