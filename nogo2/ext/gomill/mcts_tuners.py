...
class Tree(object):
    ...
    def __init__(self, splits, max_depth, exploration_coefficient,
                 initial_visits, initial_wins, parameter_formatter):
        self.splits = splits
    ...
    def describe(self):
        ...
        def describe_node(node, choice_path):
            ...
            return "%s %s %.3f %3d" % (choice_s, parameters, node.value,
    ...
    def summarise(self, out, summary_spec):
        ...
        def describe_node(node, choice_path):
            ...
            return "%s %-40s %.3f %3d" % (choice_s, parameters, node.value,
                                          node.visits - self.initial_visits)

        def most_visits(xxx_todo_changeme):
            (child_index, node) = xxx_todo_changeme
            return node.visits
        ...
        for i, n in enumerate(summary_spec):
            ...
            for path, node in sorted(
                    nlargest(
                        n, this_generation, key=most_visits)):
...
class Simulation(object):
    ...
    def _choose_action(self, node):
        ...
        def urgency(xxx_todo_changeme1):
            (i, child) = xxx_todo_changeme1
...
class Greedy_simulation(Simulation):
    ...
    def _choose_action(self, node):
        def wins(xxx_todo_changeme2):
            (i, node) = xxx_todo_changeme2
            return node.wins
...
class Mcts_tuner(Competition):
    ...
    global_settings = (
        Competition.global_settings + competitions.game_settings + [
            Setting(
                'number_of_games', allow_none(interpret_int), default=None),
            Setting('candidate_colour', interpret_colour),
            Setting(
                'log_tree_to_history_period',
                allow_none(interpret_positive_int),
                default=None),
            Setting(
                'summary_spec',
                interpret_sequence_of(interpret_int),
                default=(30, )),
            Setting(
    ...
    tree_settings = [
        Setting(
            'max_depth', interpret_positive_int, default=1),
    ...
    def initialise_from_control_file(self, config):
        ...
        try:
            ...
            raise ControlFileError(str(e))
        self.tree = Tree(
            splits=[pspec.split for pspec in self.parameter_specs],
    ...
    def format_optimiser_parameters(self, optimiser_parameters):
        return self.format_engine_parameters(
            self.scale_parameters(optimiser_parameters))
    ...
    def _write_main_report(self, out):
        ...
        if self.number_of_games is None:
            ...
            print(
                "%d/%d games played" % (games_played, self.number_of_games),
                file=out)
        print(file=out)
    ...
    def write_screen_report(self, out):
        ...
        if self.outstanding_simulations:
            ...
            for game_id, simulation in to_show:
                print(
                    "game %s: %s" % (game_id, simulation.describe()), file=out)
