...
from gomill.competitions import (Competition, CompetitionError,
                                 ControlFileError)
...
class Allplayall(tournaments.Tournament):
    ...
    def control_file_globals(self):
        ...
        result.update({'Competitor': Competitor_config, })
        return result
    ...
    def initialise_from_control_file(self, config):
        ...
        matchup_settings = [
            ...
            'rounds', allow_none(interpret_int), default=None), ]
        try:
            matchup_parameters = load_settings(matchup_settings, config)
        except ValueError as e:
            raise ControlFileError(str(e))
        matchup_parameters['alternating'] = True
        matchup_parameters['number_of_games'] = matchup_parameters.pop(
            'rounds')
        ...
        for c1_i, c1 in enumerate(self.competitors):
            for c2 in self.competitors[c1_i + 1:]:
                try:
                    m = self.make_matchup(
                        self._get_matchup_id(c1, c2), c1.player, c2.player,
                        matchup_parameters)
                except Exception as e:
    ...
    def count_games_played(self):
        ...
        return sum(len(l) for l in list(self.results.values()))
    ...
    def write_screen_report(self, out):
        ...
        if expected is not None:
            print(
                "%d/%d games played" % (self.count_games_played(), expected),
                file=out)
        else:
        ...
        for c2_i, c2 in enumerate(self.competitors):
            ...
            for c1_i, c1 in enumerate(self.competitors):
                ...
                ms = tournament_results.Matchup_stats(self.results[matchup.id],
                                                      player_x, player_y)
                column_values.append("%s-%s" % (format_float(ms.wins_1),
