...
from gomill.competitions import (Competition, NoGameAvailable,
                                 CompetitionError, ControlFileError,
                                 Player_config)
...
def update_distribution(distribution, elites, step_size):
    ...
    for i in range(distribution.dimension):
        ...
        new_mean = (elite_mean * step_size + old_mean * (1.0 - step_size))
        new_var = (elite_var * step_size + old_var * (1.0 - step_size))
...
parameter_settings = [
    ...
    Setting(
        'transform', interpret_callable, default=float),
    Setting(
        'format', interpret_8bit_string, default=None),
]
...
class Cem_tuner(Competition):
    ...
    def control_file_globals(self):
        ...
        result.update({'Parameter': Parameter_config, })
        return result

    global_settings = (
        Competition.global_settings + competitions.game_settings + [
    ...
    def initialise_from_control_file(self, config):
        ...
        competitions.validate_handicap(self.handicap, self.handicap_style,
                                       self.board_size)

        if not 0.0 < self.elite_proportion < 1.0:
            raise ControlFileError(
                "elite_proportion out of range (0.0 to 1.0)")
        ...
        try:
            ...
            raise ControlFileError("opponent: unknown player %s" %
                                   specials['opponent'])
        ...
        for i, parameter_spec in enumerate(specials['parameters']):
            ...
            if pspec.code in seen_codes:
                raise ControlFileError("duplicate parameter code: %s" %
                                       pspec.code)
    ...
    def _set_scheduler_groups(self):
        self.scheduler.set_groups((i, self.batch_size)
                                  for i in range(self.samples_per_generation))
    ...
    def reset_for_new_generation(self):
        ...
        self.sample_parameters = [
            get_sample() for _ in range(self.samples_per_generation)
        ]
    ...
    def format_optimiser_parameters(self, optimiser_parameters):
        return self.format_engine_parameters(
            self.transform_parameters(optimiser_parameters))
    ...
    def make_candidate(self, player_code, engine_parameters):
        ...
        try:
            ...
            raise CompetitionError("error from make_candidate()\n%s" %
                                   compact_tracebacks.format_traceback(skip=1))
        if not isinstance(candidate_config, Player_config):
            raise CompetitionError("make_candidate() returned %r, not Player" %
                                   candidate_config)
        try:
            candidate = self.game_jobs_player_from_config(player_code,
                                                          candidate_config)
        except Exception as e:
            raise CompetitionError(
    ...
    def prepare_candidates(self):
        ...
        for candidate_number, optimiser_params in \
            ...
            candidate_code = self.make_candidate_code(self.generation,
                                                      candidate_number)
    ...
    def finish_generation(self):
        ...
        elite_count = max(
            1, int(self.elite_proportion * self.samples_per_generation + 0.5))
        ...
        self.log_history("")
        elite_samples = [
            self.sample_parameters[index]
            for (wins, index) in sorter[:elite_count]
        ]
        self.distribution = update_distribution(self.distribution,
                                                elite_samples, self.step_size)
    ...
    def write_screen_report(self, out):
        ...
        if self.generation == self.number_of_generations:
            ...
            print(
                "distribution for generation %d:" % self.generation, file=out)
