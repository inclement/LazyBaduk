...
_player_settings = [
    ...
    Setting(
        'cwd', allow_none(interpret_8bit_string), default=None),
    Setting(
        'environ',
        allow_none(
        ...
        default=None),
    Setting(
        'is_reliable_scorer', interpret_bool, default=True),
    Setting(
        'allow_claim', interpret_bool, default=False),
    Setting(
        'gtp_aliases',
        allow_none(
            interpret_map_of(interpret_8bit_string, interpret_8bit_string)),
    ...
    Setting(
        ...
        allow_none(interpret_sequence),
        defaultmaker=list),
    Setting(
        'discard_stderr', interpret_bool, default=False),
]
...
class Competition(object):
    ...
    global_settings = [
        Setting(
            'description',
            allow_none(interpret_as_utf8_stripped),
            default=None),
    ]
    ...
    def initialise_from_control_file(self, config):
        ...
        for name, value in list(to_set.items()):
        ...
        for player_code, player_config in specials['players']:
            try:
                player = self.game_jobs_player_from_config(player_code,
                                                           player_config)
            ...
            for v in config['startup_gtp_commands']:
                try:
                    if isinstance(v, str):
                        ...
                        words = list(v)
                    if not all(
                            gtp_controller.is_well_formed_gtp_word(word)
                            for word in words):
                        raise Exception
                except Exception:
...
game_settings = [
    ...
    Setting(
        'handicap', allow_none(interpret_int), default=None),
    Setting(
        'handicap_style', interpret_enum('fixed', 'free'), default='fixed'),
    Setting(
        'move_limit', interpret_positive_int, default=1000),
    Setting(
        'scorer', interpret_enum('internal', 'players'), default='players'),
    Setting(
        'internal_scorer_handicap_compensation',
        interpret_enum('no', 'full', 'short'),
        default='full'),
]
