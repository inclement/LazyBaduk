...
def serialise_ARLN_list(values, context):
    ...
    """
    return [
        "%s:%s" % (serialise_point(p1, context), serialise_point(p2, context))
        for p1, p2 in values
    ]
...
def serialise_LB_list(values, context):
    ...
    """
    return [
        "%s:%s" % (serialise_point(point, context),
...
_property_types_by_name = {
    ...
    'point_elist': _make_property_type(
        'point_list', allows_empty_list=True),
