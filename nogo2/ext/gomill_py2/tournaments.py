...
matchup_settings = competitions.game_settings + [
    Setting(
        'alternating', interpret_bool, default=False),
    Setting(
...
class Matchup(tournament_results.Matchup_description):
    ...
    def __init__(self, matchup_id, player_1, player_2, parameters, name,
                 event_code):
        self.id = matchup_id
        ...
        self._game_id_template = (
            "%s_" + competitions.leading_zero_template(self.number_of_games))
...
class Tournament(Competition):
    ...
    def _check_results(self):
        ...
        for matchup in self.matchup_list:
            ...
            if seen_players != expected_players:
                raise CompetitionError("existing results for matchup %s "
                                       "are inconsistent with control file:\n"
                                       "result players are %s;\n"
                                       "control file players are %s" %
                                       (matchup.id, ",".join(seen_players),
                                        ",".join(expected_players)))
    ...
    def _set_scheduler_groups(self):
        self.scheduler.set_groups([(m.id, m.number_of_games)
                                   for m in self.matchup_list] + [(
    ...
    def write_matchup_report(self, out, matchup, results):
        ...
        ms = tournament_results.Matchup_stats(results, matchup.player_1,
                                              matchup.player_2)
    ...
    def get_tournament_results(self):
        return tournament_results.Tournament_results(self.matchup_list,
                                                     self.results)
