...
class Matchup_description(object):
    ...
    def describe_details(self):
        ...
        if self.handicap is not None:
            s += "handicap: %s (%s)   " % (self.handicap, self.handicap_style)
        s += "komi: %s" % self.komi
        return s
...
class Tournament_results(object):
    ...
    def get_matchup_stats(self, matchup_id):
        ...
        ms = Matchup_stats(self.results[matchup_id], matchup.player_1,
                           matchup.player_2)
...
class Matchup_stats(object):
    ...
    def calculate_colour_breakdown(self):
        ...
        if self.played_1w == 0 and self.played_2b == 0:
            ...
            self.wins_1b = sum(r.winning_player == player_1 and
                               r.winning_colour == 'b' for r in results) + js
            self.wins_1w = sum(r.winning_player == player_1 and
                               r.winning_colour == 'w' for r in results) + js
            self.wins_2b = sum(r.winning_player == player_2 and
                               r.winning_colour == 'b' for r in results) + js
            self.wins_2w = sum(r.winning_player == player_2 and
                               r.winning_colour == 'w' for r in results) + js
...
def make_matchup_stats_table(ms):
    ...
    i = t.add_column(align='right')
    t.set_column_values(i,
                        [pct(ms.wins_1, ms.total), pct(ms.wins_2, ms.total)])
    ...
    if ms.alternating:
        ...
        t.set_column_values(i, [
            pct(ms.wins_1b, ms.played_1b), pct(ms.wins_2b, ms.played_2b),
        ...
        t.set_column_values(i, [
            pct(ms.wins_1w, ms.played_1w), pct(ms.wins_2w, ms.played_y2),
